  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await apiClient.post<any>('/table-bookings', {
        ...
        specialRequests: `Specialty: ${SPECIALTIES.find(s => s.id === selectedSpecialty)?.label || selectedSpecialty}${symptoms.trim() ? `. Symptoms: ${symptoms.trim()}` : ''}`,
      });
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        {isLoading ? (
          ...
          <FlatList data={filteredClinics} keyExtractor={(item) => item._id || item.id} renderItem={renderStoreCard} contentContainerStyle={styles.storeList} showsVerticalScrollIndicator={false} />
        )}
      </SafeAreaView>
    );
  }
